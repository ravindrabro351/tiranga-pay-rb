...
function normalizePaymentReference(v){return String(v??'').normalize('NFKC').replace(/[\u200B-\u200D\uFEFF]/g,'').trim().replace(/\s+/g,'');}
function isValidPaymentReference(v){const s=normalizePaymentReference(v);return s.length>=3&&s.length<=100&&/[A-Za-z0-9]/.test(s)&&!/[\u0000-\u001F\u007F]/.test(s);}
...
async function submitPayment(planKey){
  const utr=normalizePaymentReference($('paymentUtr').value); if(!isValidPaymentReference(utr)) throw Error('UTR / Transaction ID kam se kam 3 characters ka enter karein. Copy-paste ke hidden spaces automatically clean ho jayenge.');
