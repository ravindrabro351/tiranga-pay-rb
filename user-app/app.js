...
async function logUserActivity(type,title,message,extra={}){
 try{await addDoc(collection(db,"activityEvents"),{uid:user?.uid||auth.currentUser?.uid||"",email:auth.currentUser?.email||"",type,title,message,...extra,createdAt:serverTimestamp()})}catch(e){console.warn("Activity log failed",e)}
}
...
$("registerBtn").onclick=async()=>{
 try{
  ...
  await logUserActivity("registration","New User Registered",username+" created a Tiranga Pay account.",{phone});await signOut(auth);$("showLogin").click();toast("Registration successful.");
 }catch(e){toast(e.message)}
};
$("loginBtn").onclick=async()=>{try{const c=await signInWithEmailAndPassword(auth,$("loginEmail").value.trim(),$("loginPassword").value);user=c.user;await logUserActivity("login","User Login",c.user.email+" logged in.")}catch(e){toast(e.message)}}
...
$("submitWithdrawalBtn").onclick=async()=>{try{if(!unlocked())throw Error("Account running होना चाहिए.");const amount=Number($("withdrawAmount").value),destination=$("withdrawDestination").value.trim();if(!amount||amount<=0||!destination)throw Error("Valid details भरें.");if(amount>availableWithdrawable())throw Error("Insufficient eligible balance.");await addDoc(collection(db,"withdrawals"),{uid:user.uid,userId:p.userId,username:p.username,amount,destination,status:"pending",createdAt:serverTimestamp()});await logUserActivity("withdrawal","New Withdrawal Request","₹"+amount+" withdrawal requested.",{destination});toast("Withdrawal request submitted.")}catch(e){toast(e.message)}}
