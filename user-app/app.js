...
function bindModal(){
  ...
  $('submitPayment')?.addEventListener('click',()=>{const b=$('submitPayment');if(!b||b.disabled)return;const plan=b.dataset.plan;b.disabled=true;b.textContent='Submitting...';submitPayment(plan).catch(e=>{toast(e.message);if(document.body.contains(b)){b.disabled=false;b.textContent='Submit Payment';}});});
