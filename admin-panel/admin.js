...
function withdrawalCard(w){const u=users[w.uid]||{},displayAmount=Number(w.requestValueInr??w.amount??0),dest=w.type==='crypto'?`${String(w.details?.asset||'').toUpperCase()} • ${w.details?.network||''} • ${String(w.details?.wallet||'').slice(0,12)}…`:(w.type==='upi'?w.details?.upi:`${w.details?.bank||''} • ****${String(w.details?.account||'').slice(-4)}`);return `<article class="payment-card"><div class="payment-main"><b>${esc(u.username||w.username||w.email||w.uid||'User')} • ${money(displayAmount)}</b><small>${esc(w.withdrawalId||w.id)} • ${esc(w.type||'bank')} • ${esc(dest||'')}</small>${w.type==='crypto'?`<small>Wallet: ${esc(w.details?.wallet||'')} • Network: ${esc(w.details?.network||'')}</small>`:''}<div class="payment-meta"><span>${dt(w.createdAt)}</span><span class="pill ${w.status==='success'||w.status==='paid'?'green':w.status==='rejected'?'red':'orange'}">${esc(w.status||'pending')}</span></div>${w.rejectReason?`<div class="danger-note">Reason: ${esc(w.rejectReason)}</div>`:''}${w.referenceId?`<div class="success-note">Reference: ${esc(w.referenceId)}</div>`:''}</div>${w.status==='pending'?`<div class="row-actions"><button class="tiny green" data-wd-action="success" data-path="${esc(w._path)}">Success</button><button class="tiny red" data-wd-action="reject" data-path="${esc(w._path)}">Reject</button></div>`:''}</article>`;}
...
async function reviewWithdrawal(path,action){const w=findWithdrawalByPath(path);if(!w||w.status!=='pending')return toast('Only pending withdrawal can be processed.');const uid=w.uid,u=users[uid];if(!uid||!u)return toast('User not found.');try{showLoading(true);if(action==='reject'){const reason=prompt('Withdrawal reject reason','Details could not be verified')||'Rejected by Admin';const refund=Number(w.amount||0),refundUpdates={};refundUpdates[`${path}/status`]='rejected';refundUpdates[`${path}/rejectReason`]=reason;refundUpdates[`${path}/reviewedAt`]=now();refundUpdates[`${path}/reviewedBy`]=me.uid;refundUpdates[`${path}/refunded`]=true;await update(ref(db),refundUpdates);await userActivity(uid,'withdrawal','Withdrawal Rejected',`${money(Number(w.requestValueInr??w.amount||0))} • ${reason}`);await audit('WITHDRAWAL_REJECTED',{uid,id:w.id,amount:w.amount,reason});await adminActivity('Withdrawal rejected',`${userLabel(uid)} • ${money(w.amount)}`);toast('Withdrawal rejected.');}else{const amount=Number(w.requestValueInr??w.amount??0);if(amount<=0)throw Error('Invalid withdrawal amount.');const referenceId=prompt('Payment / Reference ID (optional)','')||'ADMIN-'+Date.now().toString().slice(-10);const txid='WDRTX-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase();const updates={};updates[`${path}/status`]='success';updates[`${path}/referenceId`]=referenceId;updates[`${path}/completedAt`]=now();updates[`${path}/reviewedAt`]=now();updates[`${path}/reviewedBy`]=me.uid;updates[`transactions/${uid}/${txid}`]={transactionId:txid,title:'Withdrawal Successful',type:'withdrawal',amount,status:'completed',source:'withdrawal',withdrawalId:w.withdrawalId||w.id,referenceId,createdAt:now(),createdBy:me.uid};await update(ref(db),updates);await userActivity(uid,'withdrawal','Withdrawal Successful',`${money(amount)} • Reference ${referenceId}`);await audit('WITHDRAWAL_SUCCESS',{uid,id:w.id,amount,referenceId,txid});await adminActivity('Withdrawal successful',`${userLabel(uid)} • ${money(amount)} • ${referenceId}`);toast('Withdrawal marked successful.');}}catch(e){console.error(e);toast(e.message)}finally{showLoading(false)}}
