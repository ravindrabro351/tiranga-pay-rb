...
async function addLedger(){
  ...
  showLoading(true);try{const batchId='BAT-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase(),base=now(),updates={};
    ...
    if(type==='credit'){updates[`users/${uid}/balance`]=Number((oldBalance+total+autoCommission).toFixed(2));updates[`users/${uid}/commission`]=Number((oldCommission+autoCommission).toFixed(2));updates[`users/${uid}/withdrawableBalance`]=Number((Number(u.withdrawableBalance||oldCommission)+autoCommission).toFixed(2));if(autoCommission>0){updates[`users/${uid}/commissionRunStartedAt`]=base;}}if(type==='debit')updates[`users/${uid}/balance`]=oldBalance-total;if(type==='commission'){updates[`users/${uid}/commission`]=oldCommission+total;updates[`users/${uid}/balance`]=oldBalance+total;updates[`users/${uid}/withdrawableBalance`]=Number(u.withdrawableBalance||oldCommission)+total;updates[`users/${uid}/commissionRunStartedAt`]=base;}
...
async function addCombinedBatch(){
  ...
  showLoading(true);try{const batchId='BAT-'+Date.now().toString(36).toUpperCase()+'-'+Math.random().toString(36).slice(2,6).toUpperCase(),base=now(),updates={};
    ...
    /* Commission-run marker is persisted when the combined auto debit/credit + commission flow starts. */ updates[`users/${uid}/commission`]=Number((oldCommission+commissionTotal).toFixed(2)); updates[`users/${uid}/withdrawableBalance`]=Number((Number(u.withdrawableBalance||oldCommission)+commissionTotal).toFixed(2)); updates[`users/${uid}/commissionRunStartedAt`]=base; updates[`ledgerBatches/${batchId}`]={batchId,uid,userCode:u.userCode||'',username:u.username||'',type:'combined',fund,totalCredit:total,totalDebit:total,totalCommission:Number(commissionTotal.toFixed(2)),rate,repeatCount:count,title,createdAt:base,createdBy:me.uid,status:'completed'};
